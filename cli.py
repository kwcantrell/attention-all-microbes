...
def _create_dataset(
    ...
    metadata = pd.read_csv(m_metadata_file, sep="\t", index_col=0)
    ...
    regression_data = extract_col(metadata, m_metadata_column, output_dtype=np.float32)
    regression_dataset, mean, std = convert_to_normalized_dataset(
        regression_data, p_normalize
    )
...
def fit_regressor(
    ...
    training_dataset = batch_dataset(
        training, p_batch_size, repeat=p_repeat, shuffle=True
    )
    val_dataset = batch_dataset(
        dataset_obj["val"], p_batch_size, repeat=1, shuffle=False
    )
