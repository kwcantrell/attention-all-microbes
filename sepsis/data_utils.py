...
def convert_table_to_dataset(table):
    ...
    get_inputs = lambda x: (tf.squeeze(tf.gather(o_ids, x.indices)), tf.cast(x.values, dtype=tf.float32))
...
def train_val_split(dataset: tf.data.Dataset,
    ...
    return (
        batch_dataset(training_dataset, batch_size, shuffle=True),
        batch_dataset(validation_dataset, batch_size, shuffle=False))