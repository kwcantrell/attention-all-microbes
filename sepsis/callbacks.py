...
from aam.losses import denormalize, real_feature_mask
...
@tf.keras.saving.register_keras_serializable(
    package="plot_feature_confidence"
)
def plot_feature_confidence(
    dataset,
    model,
    fname
):
    output_dict = model.predict(dataset)

    data = feature_confidences(max_token, tokens, feature_embeddings)
    plot = sns.violinplot(
        data,
        hue="Class",
        y="Confidence",
    ...
    plot.set(title="Feature Confidence")
    plt.savefig(fname)
    plt.close()
