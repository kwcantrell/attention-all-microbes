...
#         # set zero elements to  1 / (depth + 1)
#         zero_indices = tf.where(dense_counts == 0.0)
#         updates = tf.ones(tf.shape(zero_indices)[0], dtype=tf.float32) / (depth + 1.0)
#         dense_counts = tf.tensor_scatter_nd_update(
#             dense_counts, indices=zero_indices, updates=updates
#         )

#         # normalize counts
#         dense_counts = tf.math.log(dense_counts)
