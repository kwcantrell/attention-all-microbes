...
class UnifracEncoderV2(tf.keras.layers.Layer):
    def __init__(
        self,
        output_dim: int,
        token_limit: int,
    ...
    def _unifrac_embeddings(self, tensor, mask=None, training=False):
        encoder_pred = tf.reduce_sum(tensor, axis=1) / tf.reduce_sum(mask, axis=1)
    ...
    def call(
        ...
        unifrac_embeddings = self._unifrac_embeddings(
            asv_embeddings, attention_mask, training=training
        )

        print("UnifracEncoderV2 exit...", self.trainable)
