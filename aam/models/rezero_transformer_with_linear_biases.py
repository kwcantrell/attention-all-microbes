...
@tf.keras.utils.register_keras_serializable(package="ReZeroTransformerLinearBiases")
class ReZeroTransformerLinearBiases(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if output_range is not None:
            logging.warning(
                "`output_range` is avaliable as an argument for `call()`.The `output_range` as __init__ argument is deprecated."
            )
    ...
    def build(self, input_shape):
        ...
        if policy.name == "mixed_bfloat16":
            ...
            policy = tf.float32
        self._inner_activation_layer = tf.keras.layers.Activation(self._inner_activation, dtype=policy)
        ...
        if self._share_rezero:
            ...
            self._rezero_a_ffn = self.add_weight(
                name="rezero_alpha_ffn", initializer=tf.keras.initializers.Zeros(), trainable=True, dtype=tf.float32
            )
    ...
    def get_config(self):
        config = {
            ...
            "share_rezero": self._share_rezero,
    ...
    def reset_rezero(self):
        self._rezero_a.assign(0.0)
        if not self._share_rezero:
            self._rezero_a_ffn.assign(0.0)
    ...
    def call(self, inputs, output_range: Optional[tf.Tensor] = None) -> tf.Tensor:
        ...
        attention_output = self._attention_layer(query=target_tensor, value=key_value, attention_mask=attention_mask)
        ...
        layer_output = attention_output + tf.cast(self._rezero_a_ffn * layer_output, tf.float32)
        if self._use_layer_norm:
