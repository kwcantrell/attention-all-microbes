...
class PairwiseLoss(tf.keras.losses.Loss):
    ...
    def call(self, y_true, y_pred):
        ...
        mean = tf.reduce_mean(valid_differences)
        std = tf.math.reduce_std(valid_differences)
        difficult_mask = valid_differences > (mean - std)
        non_outlier_mask = valid_differences < (mean + 3 * std)

        return tf.reduce_mean(valid_differences[difficult_mask & non_outlier_mask])
