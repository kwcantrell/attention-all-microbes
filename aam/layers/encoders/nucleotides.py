...
class NucleotideEmbedding(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        self.nucleotides = tf.Variable(
            tf.zeros(shape=(self.max_bp, 6), dtype=tf.float32),
            trainable=False,
            dtype=tf.float32,
        )

        def _add_random_sequences_and_mask(
            seq, seeds, rclr, unormalized_log_prob, range
        ):
            ...
            nucleotides = tf.reduce_sum(
                tf.multiply(
                    tf.one_hot(seq, depth=6, axis=-1, dtype=tf.float32),
            ...
            seq = tf.pad(seq, [[0, 0], [0, 1], [0, 0]], constant_values=6)
            ...
            rclr_noise = tf.divide(
                ...
                tf.cast(tf.reduce_prod(tf.shape(rclr)[1]), dtype=tf.float32),
            )
            ...
            random_mask = tf.random.stateless_binomial(
                tf.shape(seq),
                seeds[:, 3],
                tf.ones_like(seq, dtype=tf.float32),
                [1 - 0.1],
    ...
    def call(self, inputs, training=None):
        ...
        if training:
            ...
            seq, unmasked_sequences, rclr, unormalized_log_prob = (
                self._add_random_sequences_and_mask(
                    seq, seeds, rclr, unormalized_log_prob, range
                )
            )
            seq = tf.cast(seq, dtype=tf.int64)
            self.nucleotides.assign(unormalized_log_prob, read_value=False)
        else:
            seq = tf.pad(seq, [[0, 0], [0, 1], [0, 0]], constant_values=6)
...
class PCAProjector(tf.keras.layers.Layer):
    def __init__(self, hidden_dim, num_heads, num_layers, dropout, **kwargs):
        ...
        self.ff = tf.keras.layers.Dense(hidden_dim, activation="relu", use_bias=True)
        self.norm = tf.keras.layers.LayerNormalization()
        self.dropout_layer = tf.keras.layers.Dropout(dropout)
        self.point = tf.keras.layers.Dense(1)
        self.point2 = tf.keras.layers.Dense(1, activation="relu", use_bias=True)
        self.ff2 = tf.keras.layers.Dense(hidden_dim, activation="relu", use_bias=True)
...
class MultiHeadPCAProjection(tf.keras.layers.Layer):
        ...
        self.linear_up_scale = tf.keras.Sequential(
            [
                tf.keras.layers.Dense(
                    self.hidden_dim, activation="relu", use_bias=True
                ),
                tf.keras.layers.Dense(head_size, activation="relu", use_bias=True),
            ]
        )
    ...
    def init_proj(reshape, first_transp, second_transp, second_reshape):
        ...
        def compute_proj(X):
            ...
            proj = tf.math.softmax(
                tf.divide(
                    pca, tf.math.sqrt(tf.cast(tf.shape(X)[-2], dtype=tf.float32))
                ),
                axis=-1,
            )
...
class ReadHead(tf.keras.layers.Layer):
    def __init__(self, max_bp, hidden_dim, num_heads, num_layers, output_dim, **kwargs):
        ...
        self.reg_out = tf.keras.layers.Dense(self.output_dim, use_bias=True)
        self.attention_out = tf.keras.layers.Dense(max_bp, use_bias=True)
    ...
    def call(self, inputs, training=None):
        ...
        attention_out = self.nuc_out(
            tf.expand_dims(self.attention_out(output[:, :-1, :]), axis=-1)
        )
