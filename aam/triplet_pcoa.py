...
def compute_sample_distances(sample_encodings):
    sample_distances = np.square(
        sample_encodings[np.newaxis, :, :] - sample_encodings[:, np.newaxis, :]
    )
...
anchor_group_center = group_centers[groups[0]]["center"]
anchor_group_unit_center = anchor_group_center
anchor_group_indices = group_centers[groups[0]]["indices"]
