...
class TripletGeneratorV2(tf.keras.utils.Sequence):
    ...
    def _batch_data(self, batch_sample_ids, weights):
        ...
        asv_counts /= self.rarefy_depth
        asv_counts[asv_counts == 0] = 1 / (self.rarefy_depth + 1)
        asv_counts = np.log(asv_counts)
        asv_counts_mean = np.mean(asv_counts, axis=1, keepdims=True)
        asv_counts_std = np.std(asv_counts, axis=1, keepdims=True)
        asv_counts = (asv_counts - asv_counts_mean) / asv_counts_std
