...
def convert_to_tf_dataset(data):
    ...
    get_id = lambda x: (tf.gather(o_ids, x.indices), tf.cast(PLACEHOLDER))
...
def gotu_encode_and_convert(data):
    ...
    gotu_dict = {int(t): gotu for t, gotu in zip(list(tf.squeeze(gotu_tokens, axis=-1).numpy()), gotu_list)}
...
def combine_all_datasets(asv_dataset, gotu_dataset):
    return tf.data.Dataset.zip(asv_dataset, gotu_dataset).prefetch(tf.data.AUTOTUNE)
...
def generate_train_val_sets(dataset: tf.data.Dataset, train_split: int, batch_size: int, shuffle: int):
...
def create_training_dataset(gotu_fp: str, asv_fp: str, train_split: int):
    ...
    training_batched, val_batched = generate_train_val_sets(combined_dataset, train_split, 8, 100)
