...
from attention_regression.layers import FeatureEmbedding, FeatureLoadings, Regressor
...
def _construct_model(
    ...
    feature_input = tf.keras.Input(shape=[None], dtype=tf.string, name="feature")
    rclr_input = tf.keras.Input(shape=[None], dtype=tf.float32, name="rclr")
    ...
    binary_loadings = FeatureLoadings(
        ...
        dropout=dropout,
        output_dim=(
            3 if p_feature_attention_method == "add_features" else len(ids) + 1
        ),
        name="FeatureLoadings",
    )
    output_embeddings = binary_loadings(output_embeddings)

    regressor = Regressor(ff_d_model, pca_heads, 2, 1, 32, 0.1)
...
class AttentionRegression(tf.keras.Model):
    ...
    def make_call_function(self):
        ...
        def one_step(inputs, training=None):
            emb_outputs = self.feature_emb(tf.nest.flatten(inputs), training=training)
            ...
            output_embeddings = self.binary_loadings(
                output_embeddings, training=training
            )
    ...
    def _add_feature_loss(self, model_outputs):
        ...
        max_added = tf.repeat(
            tf.reduce_max(
                tf.reduce_sum(tf.cast(tf.equal(labels, 1), dtype=tf.int64), axis=1)
            ),
        ...
        embeddings, labels = tf.vectorized_map(
            self.select_equal_class, [embeddings, labels, max_added]
        )

        attention_loss = tf.reduce_mean(
            self.attention_loss(labels, embeddings), axis=-1
        )
    ...
    def _add_mask_loss(self, model_outputs):
        ...
        token_mask = tf.math.not_equal(feature_tokens, sample_tokens)
        batch_counts = tf.squeeze(
            tf.reduce_sum(tf.cast(token_mask, dtype=tf.float32), axis=-1)
        )
        batch_counts = tf.repeat(batch_counts, tf.cast(batch_counts, dtype=tf.int64))
        ...
        attention_loss = tf.divide(
            tf.reduce_sum(
                tf.divide(
                    self.attention_loss(
                        tf.one_hot(labels, depth=tf.shape(embeddings)[1]), embeddings
                    ),
                    batch_counts,
                )
            ),
    ...
    def _attention_loss(self, model_outputs):
        if self.feature_attention_method == "add_features":
            ...
            raise Exception(f"Invalid attention method {self.feature_attention_method}")
    ...
    def metrics(self):
        ...
        return [self.loss_tracker, self.confidence_tracker, self.mae_metric]
    ...
    def feature_confidences(self, dataset):
        ...
        for c in conf_dict["classes"]:
            ...
            for k in keys:
                if k == "age":
                    conf_dict[c][k] = {
                        "true_age": tf.zeros(
                            self.feature_emb.total_tokens, dtype=tf.float32
                        ),
        ...
        def map_to_tokens(batch_tensor, class_mask, class_tokens):
            # extract class info from batch tensor
            class_tensor = tf.multiply(
                batch_tensor, tf.cast(class_mask, dtype=tf.float32)
            )
        ...
        def _process_batch(x, y):
            ...
            model_outputs["predicted_age"] = _normalize_age(model_outputs["regression"])
            model_outputs["true_age"] = _normalize_age(
                tf.expand_dims(y["reg_out"], axis=-1)
            )
            ...
            for c in conf_dict["classes"]:
                ...
                conf_dict[c]["confidence"] += map_to_tokens(
                    batch_confidence, class_mask, class_tokens
                )

                ones = tf.ones_like(model_outputs["tokens"], dtype=tf.float32)

                conf_dict[c]["age"]["true_age"] += map_to_tokens(
                    tf.multiply(model_outputs["true_age"], ones),
                    class_mask,
                    class_tokens,
                )
                conf_dict[c]["age"]["predicted_age"] += map_to_tokens(
                    tf.multiply(model_outputs["predicted_age"], ones),
                    class_mask,
                    class_tokens,
                )

                conf_dict[c]["count"] += map_to_tokens(ones, class_mask, class_tokens)
    ...
    def get_config(self):
        ...
        config = {
            ...
            "regressor": tf.keras.saving.serialize_keras_object(self.regressor),
        }
