...
class FeatureEmbedding(tf.keras.layers.Layer):
    def __init__(
        ...
        **kwargs,
        ...
        self.tokens = tf.range(tf.constant(len(emb_vocab) + 1, dtype=tf.int64))
        ...
        self.ff = tf.keras.Sequential(
            [
                tf.keras.layers.Dense(ff_clr, activation="relu", use_bias=True),
    ...
    def _random_tokens(self, inputs):
        ...
        # get random tokens
        random_tokens = tf.random.shuffle(self.tokens[in_sample_mask < 1])
        random_tokens = random_tokens[: tf.minimum(feature_size, features_to_add)]
        reduced_size = tf.cast(tf.shape(random_tokens)[0], dtype=tf.int64)
        random_tokens = tf.pad(random_tokens, [[feature_size - reduced_size, 0]])
    ...
    def _add_features(self, inputs, training=None):
        ...
        if training:
            ...
            feature_count = tf.multiply(
                tf.reduce_sum(
                    tf.cast(feature_tokens > 0, dtype=tf.float32),
                    axis=-1,
                    keepdims=True,
                ),
            ...
            feature_size = tf.repeat(
                tf.cast(tf.shape(feature_tokens)[1], dtype=tf.int64),
            ...
            random_tokens = tf.map_fn(
                self._random_tokens,
                (feature_tokens, tf.cast(feature_count, dtype=tf.int64), feature_size),
            ...
            sample_rclr = tf.add(rclr, tf.cast(random_tokens > 0, dtype=tf.float32))
        else:
    ...
    def _random_mask(self, inputs):
        ...
        feature_count = tf.reduce_sum(tf.cast(feature_mask, dtype=tf.int32))

        random_mask = tf.cast(
            tf.math.greater_equal(tf.random.uniform(shape=[feature_count]), mask_rate),
            dtype=tf.int64,
        )
        random_mask = tf.pad(
            random_mask, [[0, tf.shape(sample_tokens)[0] - feature_count]]
        )
        sample_tokens = tf.multiply(sample_tokens, random_mask)
    ...
    def _mask_features(self, inputs, training=None):
        ...
        if training:
            mask_rate = tf.repeat(
                tf.cast(self.features_add_rate, dtype=tf.float32),
                repeats=[tf.shape(feature_tokens)[0]],
                axis=0,
            )
            ...
            sample_tokens = tf.map_fn(
                self._random_mask,
                (feature_tokens, mask_rate, feature_mask),
                fn_output_signature=tf.int64,
            )
        else:
            sample_tokens = feature_tokens
    ...
    def _modify_tokens_rclr(self, inputs, training=None):
        if self.attention_method == "add_features":
            ...
            raise Exception(f"Invalid attention method {self.attention_method}")
    ...
    def call(self, inputs, training=None):
        ...
        output = tf.multiply(output, tf.expand_dims(sample_rclr, axis=-1))
        ...
        return [feature_mask, sample_tokens, output_embeddings, output_regression]
...
@tf.keras.saving.register_keras_serializable(package="FeatureLoadings")
class FeatureLoadings(tf.keras.layers.Layer):
    def __init__(self, enc_layers, enc_heads, dff, dropout, output_dim, **kwargs):
        super().__init__(**kwargs)
...
def _pca(tensor):
    non_zeros = tf.math.count_nonzero(tensor, axis=1, keepdims=True, dtype=tf.float32)
    zero_vectors = tf.cast(
        tf.greater(tf.math.count_nonzero(tensor, axis=-1, keepdims=True), 0),
        dtype=tf.float32,
    )

    mean_vector = tf.math.divide_no_nan(
        tf.reduce_sum(tensor, axis=1, keepdims=True), non_zeros
    )
    var_vector = tf.reduce_sum(
        tf.math.divide_no_nan(
            tf.math.square(tf.math.subtract(tensor, mean_vector)), non_zeros
        ),
        axis=1,
        keepdims=True,
    )
    ...
    output = tf.math.divide_no_nan(tensor - mean_vector, std_vector)
    output = tf.multiply(output, zero_vectors)

    eigen_values, eigen_vectors = tf.linalg.eigh(
        tf.matmul(output, output, transpose_a=True)
    )
    pca_transform = tf.transpose(
        tf.matmul(tf.linalg.diag(eigen_values), eigen_vectors), perm=[0, 2, 1]
    )
    return pca_transform


@tf.keras.saving.register_keras_serializable(package="PCA")
class PCA(tf.keras.layers.Layer):
    ...
    def call(self, inputs, training=None):
        ...
        new_shape = tf.reshape(
            tf.stack([shape[:-1], tf.constant([self.num_heads, self.head_size])]),
            shape=[-1],
        )
        multi_head_tensor = tf.transpose(
            tf.reshape(inputs, new_shape), perm=self.expand_perm
        )
        ...
        multi_head_tensor = tf.reshape(
            multi_head_tensor,
            shape=[shape[0], self.num_heads * self.head_size, self.head_size],
        )
        multi_head_tensor = tf.divide(
            multi_head_tensor, tf.sqrt(tf.cast(self.head_size, dtype=tf.float32))
        )
...
class Regressor(tf.keras.layers.Layer):
    def __init__(
        ...
        **kwargs,
        ...
        self.position_encoder = tfm.nlp.layers.PositionEmbedding(d_model)
        self.encoder = tfm.nlp.models.TransformerEncoder(
    ...
    def call(self, inputs, training=None):
        ...
        output = output[:, -1, :]
        output = tf.squeeze(self.regress_ff(output), axis=-1)
        return output
...
@tf.keras.saving.register_keras_serializable(package="ProjectDown")
class ProjectDown(tf.keras.layers.Layer):
    def __init__(self, emb_dim, dims, reduce_dim, **kwargs):
        super().__init__(**kwargs)
        self.emb_dim = emb_dim
        self.dims = dims
        self.reduce_dim = reduce_dim
        self.ff = tf.keras.layers.Dense(emb_dim, activation="relu", use_bias=True)
    ...
    def call(self, inputs, training=None):
        ...
        if self.reduce_dim:
            output = tf.squeeze(self.proj_down(outputs), axis=-1)
        else:
