...
from aam.data_utils import batch_dataset, get_unifrac_dataset, load_data
from aam.model_utils_current import regressor
from aam.nuc_model import TransferLearnNucleotideModel, UnifracModel
...
def fit_regressor(
    ...
    core_callbacks = [
        ...
        tf.keras.callbacks.ReduceLROnPlateau(
            "loss", factor=0.5, patients=20, min_lr=0.0001
        ),
...
def transfer_learn_fit_regressor(
    ...
    core_callbacks = [
        # tboard_callback,
        tf.keras.callbacks.ReduceLROnPlateau(
            "loss", factor=0.8, patients=10, min_lr=0.0001
        ),
        tf.keras.callbacks.EarlyStopping("loss", patience=500),
        SaveModel(p_output_dir, p_report_back_after),
    ]
